...
뒷줄입니다.`;
  ...
  it("🔴 영상 자리표시자도 소제목이 아니다 (2026-09-05)", () => {
    const t = `앞줄입니다.
...
/**
 * 🔴 영상은 **있는 날만** 넣는다 (사장님 질문 2026-09-05).
 *    못 찍은 날 원고에 빈 「[영상 …]」 이 남으면 사장님이 손으로 지우셔야 한다.
 *    그래서 검사기는 영상을 **요구하지 않는다** — 없는 것이 정상이다.
 */
describe("영상 자리 — 있으면 통과, 없어도 통과", () => {
  it("영상이 없는 글은 그대로 통과한다", () => {
    assert.equal(styleFilter(GOOD), null);
  });

  it("영상 한 줄이 끼어도 통과한다", () => {
    const withVideo = GOOD.replace(
      "리프트에 올려 네 바퀴를 모두 측정했습니다.",
      `[영상 - 작업 장면]

리프트에 올려 네 바퀴를 모두 측정했습니다.`,
    );
    assert.equal(styleFilter(withVideo), null);
  });
});

/**
 * 🔴 지시문은 **영상이 있다고 알려 줬을 때만** 영상 자리를 만들게 해야 한다.
 *    이 한 줄이 빠지면 모델이 영상 없는 날에도 자리를 넣는다.
 */
describe("buildSystem — 영상은 조건부", () => {
  it("「영상이 있다고 알려 준 경우에만」이 들어 있다", () => {
    const s = buildSystem([]);
    assert.match(s, /영상이 있다고 알려 준 경우에만/);
  });
});

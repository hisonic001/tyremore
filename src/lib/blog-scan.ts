...
export async function scanFolders(onLog?: (s: string) => void, opts: ScanOpts = {}): Promise<ScanResult> {
  ...
  /** 썸네일은 sharp 로. 없으면 썸네일 없이 목록만 만든다 (화면이 회색 칸으로 보여준다) */
  ...
  try {
    sharp = ((await import("sharp")) as unknown as { default: SharpFn }).default;
  } catch {
    log("⚠️ 썸네일 도구(sharp)를 못 찾아 미리보기 없이 목록만 만듭니다");
  }

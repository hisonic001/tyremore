...
export async function finInbox(ym: string): Promise<FinInbox> {
  ...
  for (const s of dep.open) {
    ...
    put(s.dep.payerName, {
      ...
      linkDeposit: one ? { cashTxnId: s.dep.id, quoteId: one.quoteId, label: `판매 「${one.label}」와 대조` } : undefined,
    });
  }
  ...
  for (const r of a1) {
    put(r.who, {
      ...
      linkDeposit: r.cand ? { cashTxnId: r.cand.cashTxnId, quoteId: r.quoteId, label: `${r.cand.label}와 대조` } : undefined,
  ...
  for (const r of pay.rows) {
    put(r.payer, {
      ...
      payFrom: r.suggest
        ? { cashTxnId: r.id, supplier: r.suggest.supplier, label: `${r.suggest.supplier} 지급 대조` }
        : undefined,
    });
  }
  ...
  for (const dir of ["매입", "매출"] as const) {
    ...
    for (const [name, v] of byName) {
      put(name, {
        text: `${dir} 계산서 ${v.n}장 · ${won(v.sum)}원 미대조`,
        tone: "info",

 ...
 *   매입(빨강)·매출(초록) 한 화면. 사람이 누르는 건 [맞추기] [경비] [나중에] [안 봄] 넷뿐.
 *   월정산 거래처는 장 단위로 안 보고 「계산서 합 · 준 돈 · 남은 돈」 한 줄(기준일 + 시작 잔액).
 *
...
const MARK: Record<Mark, { icon: string; label: string; cls: string }> = {
  done: { icon: "✅", label: "끝", cls: "text-brand-700" },
  confirm: { icon: "🟡", label: "확인", cls: "text-amber-700" },
  hand: { icon: "✖", label: "손 필요", cls: "text-red-600" },
  wait: { icon: "⚪", label: "기다림", cls: "text-slate-400" },
};
    ...
    <div className="mt-3">
      ...
      <div className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1">
        ...
        <span className="text-xs text-slate-500">
          ...
          <span className="ml-2">
            {MARK.hand.icon} 손 필요 {book.counts.hand} · {MARK.confirm.icon} 확인 {book.counts.confirm} · {MARK.done.icon} 끝{" "}
            {book.counts.done}
          </span>
        </span>
      </div>
      ...
      <AutoDoneList book={book} ctx={ctx} />
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  <span className="flex flex-wrap items-center gap-x-2 gap-y-0.5">
                    ...
                    {p.handN > 0 && <StatusPill tone="error">손 필요 {p.handN}</StatusPill>}
                    {p.confirmN > 0 && <StatusPill tone="warn">확인 {p.confirmN}</StatusPill>}
                  </span>
      ...
      {book.past.n > 0 && (
        <div className="mt-4 flex flex-wrap items-center justify-between gap-2 rounded-card border border-slate-200 bg-white px-4 py-3 text-sm text-slate-600">
          <span>
            ...
            <span className="tabular">{won(book.past.amount)}</span>원 — 남은 돈 셈에는 안 들어가고, 원장에서 봅니다
          </span>
...
function AutoDoneList({ book, ctx }: { book: TaxBook; ctx: Ctx }) {
  ...
  return (
    <details className="mt-3 rounded-card border border-slate-200 bg-white px-4 py-2 text-sm">
      <summary className="cursor-pointer select-none py-1 font-medium text-slate-700">
        앱이 알아서 맞춘 것 <span className="tabular">{cards.length}</span>장 · 경비 <span className="tabular">{expenseN}</span>장{" "}
        <span className="text-slate-400">— 펼쳐서 확인하거나 되돌릴 수 있어요</span>
      </summary>
      {cards.length === 0 ? (
        <p className="py-2 text-slate-500">되돌릴 카드가 없습니다.</p>
      ) : (
        <ul className="divide-y divide-slate-100">
          {cards.map((c) => {
            const isRule = c.status === "expense" || c.status === "ignored";
            const k = `a:${c.id}`;
            return (
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  <button
                    ...
                    onClick={() =>
                      ctx.act(
                        k,
                        () => (isRule ? ignoreTaxInvoice(c.id, true) : undoTaxMatch(c.id)),
                        "되돌렸습니다 — 다시 정리할 목록으로 갔습니다.",
                      )
                    }
                    className="shrink-0 text-xs text-slate-500 underline underline-offset-4 disabled:opacity-40"
                  >
                    되돌리기
                  </button>
                </div>
...
/** 카드 하나를 앱 추측대로 맞추는 계획 — 어떤 액션을 부를지 + 사람에게 보여 줄 한 줄 */
...
function ConfirmLayer({ items, ctx }: { items: { card: InvoiceCard; party: PartyRow }[]; ctx: Ctx }) {
  ...
  const runAll = () =>
    start(async () => {
      ...
      // 🔴 순차 — 풀 max 3. 같은 묶음(통장 한 줄 = N장)·같은 상쇄 쌍은 한 번만 부른다.
      for (const r of rows) {
      ...
      setResult(
        errs.length === 0
          ? { tone: "success", text: `${done}장을 맞췄습니다.` }
          : { tone: "error", text: `${done}장은 맞췄고 ${errs.length}장은 못 했습니다: ${errs.join(" / ")}` },
      );
      router.refresh();
    });
    ...
    <section className="mt-4 rounded-card border border-amber-300 bg-amber-50 p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h3 className="font-semibold">
          {MARK.confirm.icon} 확인해 주세요 <span className="tabular">{rows.length}</span>장 — 체크해서 한 번에
        </h3>
      ...
      <p className="mt-1 text-xs text-amber-900">앱이 통장 줄을 하나씩 찾아 뒀습니다. 맞으면 그대로, 아니면 체크를 끄고 아래 상대 줄에서 직접 맞추세요.</p>
...
function CardView({ card: c, party, ctx }: { card: InvoiceCard; party: PartyRow; ctx: Ctx }) {
  ...
  const undo = () => ctx.act(k, () => undoTaxMatch(c.id), "되돌렸습니다 — 맞춘 기록을 풀었습니다.");
  const expense = async () => {
    if (!party.bizNo) return;
    if (
      !(await ctx.ask({
        title: `${party.name} — 경비로 정리할까요?`,
        body: `이 상대는 앞으로 경비로 자동 정리합니다 — 지금 ${openN}장.\n(잘못 눌렀으면 「앱이 알아서 맞춘 것」에서 되돌릴 수 있어요)`,
        confirmLabel: "경비로",
      }))
    )
      return;
  ...
  const later = () => ctx.act(k, () => markTaxWaiting(c.id, true), "나중에 볼 목록으로 뒀습니다 — 받을·줄 돈은 그대로 남아 있어요.");
  const ignore = async () => {
    if (
      !(await ctx.ask({
        title: "이 계산서를 안 보는 걸로 할까요?",
        body: `${md(c.d)} ${c.direction} ${won(c.total)}원을 정리 대상에서 뺍니다.\n돈을 받거나 줄 건이면 「나중에」를 쓰세요.`,
        confirmLabel: "안 봄",
        tone: "danger",
      }))
    )
      return;
    ctx.act(k, () => ignoreTaxInvoice(c.id), "안 보는 걸로 뒀습니다.");
  };
...
function bankMsg(r: { remaining?: number; shortfall?: number; netted?: boolean; applied?: number; absorbed?: number; settled?: number }) {
  const parts: string[] = [];
  if (r.netted) parts.push("반대 방향 줄로 맞췄습니다 — 수수료를 떼고 주고받은 건입니다");
  if (r.applied && r.applied > 1) parts.push(`${r.applied}줄을 합쳐 맞췄습니다`);
  ...
  if (r.shortfall) parts.push(`계산서에 ${won(r.shortfall)}원이 남았습니다 — 다른 줄을 이어서 맞추세요`);
  if (r.remaining) parts.push(`통장 줄에 ${won(r.remaining)}원이 남았습니다 (다른 계산서 몫이면 이어서 맞추세요)`);
  return parts.length ? `맞췄습니다 — ${parts.join(" · ")}.` : "맞췄습니다 — 금액이 정확히 맞습니다.";
}
    ...
    <div className="mt-2 rounded-control border border-brand-500/40 bg-brand-50/40 p-2.5">
      ...
      {mo.fixFirst && (
        <div className="mb-1.5 flex items-center justify-between gap-2 text-xs">
          <span className="min-w-0">{mo.fixFirst.label} — 서로 지움이 먼저입니다</span>
          <button
            type="button"
            disabled={ctx.pending}
            onClick={() => {
              const f = mo.fixFirst!;
              ctx.act(noteKey, () => markTaxFixPair(f.minusId, c.id), "원본과 수정 계산서를 서로 지웠습니다.");
            }}
      ...
      {mo.fix && (
        <div className="mb-1.5 flex items-center justify-between gap-2 text-xs">
          ...
          <button
            ...
            onClick={() => {
              const f = mo.fix!;
              ctx.act(noteKey, () => markTaxFixPair(c.id, f.originId), "원본과 수정 계산서를 서로 지웠습니다.");
            }}
            className="shrink-0 rounded-control bg-brand-600 px-2.5 py-1.5 font-semibold text-white active:bg-brand-700 disabled:opacity-40"
          >
            원본과 서로 지움
      ...
      {mo.bundle && (
        <div className="mb-1.5 flex items-center justify-between gap-2 text-xs">
          ...
          <button
            ...
            onClick={() => {
              const b = mo.bundle!;
              ctx.act(noteKey, () => confirmBankToTaxes(b.cashTxnId, b.invoiceIds), `계산서 ${b.invoiceIds.length}장을 통장 한 줄에 맞췄습니다.`);
            }}
            className="shrink-0 rounded-control bg-brand-600 px-2.5 py-1.5 font-semibold text-white active:bg-brand-700 disabled:opacity-40"
          >
      ...
      {showSearch ? (
        <div className={hasSuggest ? "mt-2 border-t border-dashed border-slate-300 pt-2" : ""}>
          {!hasSuggest && <p className="mb-1 text-xs text-slate-600">앱이 찾은 통장 줄이 없습니다 — 이름이나 금액으로 직접 찾아 맞추세요.</p>}
...
function MonthlyPanel({ party, s, ym, ctx }: { party: PartyRow; s: MonthlySummary; ym: string; ctx: Ctx }) {
  ...
  const save = () => {
    ...
    ctx.act(k, () => setTaxBaseline(bizNo, date, Number.isFinite(n) ? n : 0, note.trim() || null), "시작 잔액을 고쳤습니다 — 남은 돈을 다시 셌습니다.");
    setEditing(false);
  };
    ...
    <div className="text-sm">
      ...
      {editing && (
        <div className="mt-2 grid grid-cols-1 gap-2 rounded-control border border-slate-200 bg-white p-3 sm:grid-cols-3">
          ...
          <Field label="그 날 남은 돈(원)" inputMode="numeric" value={amount} onChange={(e) => setAmount(e.target.value)} hint="세무사 원장 잔액을 그대로" />
      ...
      <div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-3">
        ...
        <Stat label="남은 돈" value={s.remain} strong tone={s.remain > 0 ? "warn" : "ok"} />
      </div>
      {s.paidPast > 0 && (
      ...
      {s.remain <= 0 && <p className="mt-1 text-xs text-brand-700">남은 돈이 없어 이 달 계산서는 앱이 끝 처리합니다.</p>}

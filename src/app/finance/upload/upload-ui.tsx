...
/**
 * ⭐ 돈 관리 — 통장·법인카드 엑셀 올리기 (ERP 1단계, 2026-08-24)
 *
 *   고르기 → **미리보기 강제** → 계정 이름 적고 확정 (재고 엑셀과 같은 3단계).
...
export function FinUpload() {
  ...
  function onPick(f: File | null) {
    ...
    start(async () => {
      ...
      // 계정 이름을 전에 하나만 썼다면 그걸 미리 채워 드린다
  ...
  function onApply() {
    ...
    start(async () => {
      ...
      if (!r.ok) return setError(r.error);
      setDoneMsg(`반영했습니다 — 새로 ${r.newCount}줄 · 이미 있음 ${r.dupCount}줄`);
      setPreview(null);
      setFile(null);
    ...
    <>
      <section className="mt-4 rounded-2xl border border-slate-200 bg-white p-4">
        ...
        <p className="mt-1 text-sm text-slate-500">
          은행 거래내역 · 법인카드 이용내역(KB 확인서 · 우리카드) 엑셀을 올리면 무엇으로 읽었는지
          먼저 보여 드립니다. <strong>같은 파일을 또 올려도 두 번 계산되지 않습니다.</strong>
        </p>
        <input
          ref={input}
          type="file"

 ...
 * 자료: 부품몰 전체 카탈로그 2,927종 + 우리 재고 + 쏘카 오더 194건 + 배터리 매입 명세
 *
 * 산출 방법
 *   ① 분류별 **월 수요를 실측**한다 (앱 공임 기록 + 쏘카 오더 + 배터리 매입 실적)
 *   ② 그 수요를 **차종 가중치**로 나눠 갖는다 (우리 정비 차종 + 쏘카 차종)
 *   ③ 상위 품목만 2~3개월분 넉넉히 상비하고 나머지는 주문 대응 (부품몰 익일 배송)
 *
 * 🔴 조심한 것
 ...
 *   · 제네시스·기아 상급 전용은 무상 쿠폰·전담 AS 몫이라 뺀다 (쏘카 차종은 예외).
 *
...
/* ① 차종 수요 가중치 — 우리 정비 실적 + 쏘카 오더 (실측) */
const VEHICLE: [RegExp, number, string][] = [
  [/전차종|승용전차종|소형\s*전차종/i, 200, "전차종 범용"],
  [/포터|봉고/i, 236, "포터·봉고"],
  [/싼타페/i, 132, "싼타페"],
  [/쏘나타|소나타|\bEF\b|\bNF\b|\bYF\b|\bLF\b|DN8/i, 99, "쏘나타"],
  [/카니발/i, 96, "카니발"],
  [/스타렉스|스타리아/i, 90, "스타렉스·스타리아"],
  [/아반떼|아반테|CN7|엘란트라/i, 89, "아반떼"],
  [/그랜저|그랜져|그렌져|\bIG\b|\bHG\b|\bTG\b/i, 86, "그랜저"],
  [/셀토스/i, 76, "셀토스"],
  [/모닝/i, 63, "모닝"],
  [/레이\b/i, 62, "레이"],
  [/쏘렌토|소렌토|MQ4/i, 57, "쏘렌토"],
  [/스파크/i, 37, "스파크"],
  [/팰리세이드|펠리세이드/i, 35, "팰리세이드"],
  [/투싼|투산|NX4/i, 34, "투싼"],
  [/코나/i, 21, "코나"],
  [/캐스퍼/i, 14, "캐스퍼"],
  [/K3\b/i, 9, "K3"],
...
const GENESIS = /제네시스|G80|GV80|G90|G70|GV70|에쿠스|EQ900/i;
...
const PLAN: Record<Cat, { d: number; months: number; topN: number; cap: number; note: string }> = {
  오일필터: { d: 11.7, months: 2, topN: 12, cap: 12, note: "월 11.7건 (우리 25 + 쏘카 63, 7.5개월)" },
  에어컨필터: { d: 9.9, months: 2, topN: 12, cap: 10, note: "월 9.9건 (우리 11 + 쏘카 63)" },
  에어필터: { d: 5, months: 2, topN: 10, cap: 8, note: "월 5건 추정 — 앱 기록 0, 오일교환 동반 가정" },
  브레이크패드: { d: 1.5, months: 3, topN: 8, cap: 3, note: "월 1.5건 (우리 9건) — 한 세트가 비싸 최소만" },
};
...
async function main() {
  ...
  for (const cat of Object.keys(PLAN) as Cat[]) {
    ...
    for (const g of cands) {
      ...
      const n = Math.max(1, Math.min(p.cap, Math.round(share * p.d * p.months * 2)));
  ...
  const summary: (string | number)[][] = [
    ...
    ...order.map((k) => {
      ...
      const note = k === "배터리" ? "월 47개 (매입 실적 354개)" : PLAN[k as Cat].note;
    ...
    [],
    ["근거 ① 실측 수요", "우리 앱 공임(1~8월): 엔진오일 25 · 배터리교환 22 · 에어컨필터 11 · 브레이크패드 9 · 에어필터 0(기록 없음)"],
    ["", "쏘카 오더 194건(월 26건): 엔진오일 63 · 에어컨필터 63 · 배터리 28 — 신차라도 우리 매장에서 정비하므로 별도 최소치"],
    ["", "배터리 매입 실적 354개(월 47) — 돈이 실제로 나간 가장 확실한 신호"],
    ["근거 ② 차종 가중치", "포터·봉고 236 · 싼타페 132 · 쏘나타 99 · 카니발 96 · 스타렉스/스타리아 90 · 아반떼 89 · 그랜저 86 · 셀토스 76 · 모닝 63 · 레이 62 · 쏘렌토 57"],
    ["근거 ③ 정비 생태계", "일반보증 3년/6만·파워트레인 5년/10만 → 일반 신차는 블루핸즈/오토큐 몫, 우리 손님은 차령 4년+ 구형 위주. 제네시스는 소모품까지 무상 쿠폰, 기아 상급은 오일 쿠폰 → 전용 부품 제외 (쏘카 차종은 예외)"],
    ...
    ["확인 부탁", "에어필터는 앱 판매 기록이 0인데 재고가 170개입니다 — 실제로는 파는데 기록이 안 남는 듯합니다. 월 몇 개 나가는지 알려주시면 고치겠습니다"],
    ["확인 부탁", "브레이크패드도 앱 기록은 월 1.5건인데 재고가 128세트입니다 — 실제 회전을 알려주세요"],
  ];

...
async function main() {
  ...
  const { weeklyPayableStep } = await import("../src/lib/weekly-payables");
  const { taxPayableTotal } = await import("../src/lib/tax-recon");
  ...
  const screens: { name: string; note: string; known?: boolean; run: (ym: string) => Promise<void> }[] = [
    ...
    { name: "미지급", note: "달과 무관", run: async () => void (await payablesData()) },
    {
      name: "미지급 화면",
      known: true,
      note: "🔴 거래처 수에 비례(앱 매입 19~22 + 계산서 기준 23~24) — 둘 다 「상대 1곳당 1~2질의」 구조. 42~46이라 상한 80 아래 (2026-09-16)",
      run: async (ym) => {
        await weeklyPayableStep(ym);
        await taxPayableTotal(ym);
      },
    },
